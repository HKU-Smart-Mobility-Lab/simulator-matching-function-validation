...
def dispatch_alg_array(dispatch_observ):
    columns_name = ['order_id', 'driver_id', 'reward_units', 'order_driver_distance']
    ...
    matrix_pu_distance = np.zeros([M, N])  # pick up distance
    ...
    matrix_pu_distance[index_order, index_driver] = dispatch_observ['order_driver_distance'].values
    ...
    for m, index_driver in zip(index_existed[0].tolist(), index_existed[1].tolist()):
        dispatch_action.append([l_orders[m], l_drivers[index_driver], matrix_reward[m][index_driver],
                                matrix_pu_distance[m][index_driver]])
